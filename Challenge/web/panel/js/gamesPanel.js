...
(function() {
    ...
    function onMessage(message) {
        ...
        if (panelHasQuery(msgObject)) {
			...
			// Add the following if clause 
			
			if (panelCheckQuery(msgObject, 'games_challenge')) {
                for (idx in msgObject['results']) {
                   $('#' + msgObject['results'][idx]['key'] + 'Input').attr('placeholder', msgObject['results'][idx]['value']);
                }
            }			
			
			// End of Copy for Challenge
    ...
    function doQuery() {
		...
		sendDBKeys('games_challenge', 'challengeSettings'); // Add this line here
	...
	// Copy this whole chuck of function
	/**
     * @function challengeUpdateSetting
     * @param {String} setting
     */
    function challengeUpdateSetting(setting, l) {
        var value = $('#' + setting + 'Input').val();

        if (value.length > 0) {
            if (setting == 'attackSuccess') {
                sendDBUpdate('games_challenge', 'challengeSettings', setting, value);
            }
            ...
            if (setting == 'defaultHealth') {
                sendDBUpdate('games_challenge', 'challengeSettings', setting, value);
            }

            if (setting == 'timeoutSec') {
                sendDBUpdate('games_challenge', 'challengeSettings', setting, value);
            }

            if (setting == 'messageInterval') {
                sendDBUpdate('games_challenge', 'challengeSettings', setting, value);
            }
           ...
           setTimeout(function() { doQuery(); }, TIMEOUT_WAIT_TIME);
           setTimeout(function() { sendCommand('reloadchallenge'); }, TIMEOUT_WAIT_TIME);
        }
    }
	...
	$.challengeUpdateSetting = challengeUpdateSetting; // Add this line as well.
